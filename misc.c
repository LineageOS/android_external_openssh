...
pwcopy(struct passwd *pw)
{
	...
	if (pw->pw_gecos) copy->pw_gecos = xstrdup(pw->pw_gecos);
#endif
	...
	if (pw->pw_class) copy->pw_class = xstrdup(pw->pw_class);
#endif
	if (pw->pw_dir) copy->pw_dir = xstrdup(pw->pw_dir);
	if (pw->pw_shell) copy->pw_shell = xstrdup(pw->pw_shell);
	return copy;
}
